...
async def dispatch_promos(bot, promo_dict):
    '''
    ...
    for promo_page in promo_dict.keys():
        ...
        await bot.send_message(chat_id=credential.CHAT_ID, text=promo_page)
        # print(promo_dict[promo_page])
        # send all image url in page
        for img_url in promo_dict[promo_page]:
            # print(img_url)
            if img_url.endswith('pdf'):
                await bot.send_message(chat_id=credential.CHAT_ID, text=img_url)
            else:
                try:
                    ...
                    await bot.send_photo(chat_id=credential.CHAT_ID, photo=img_url)
...
async def runner(bot):
    ...
    async with bot:
        ...
        await dispatch_promos(bot, promo_dict)

        # query all NTUC the promos
        promo_dict = await ntuc.getNTUCPromos()
        await dispatch_promos(bot, promo_dict)
